...
from . import mdp
from .coco_robot_cfg import COCO_CFG, COCOVelocityActionsCfg
...
@configclass
class CostnavIsaaclabV2SceneCfg(InteractiveSceneCfg):
...
class ActionsCfg:
    ...
    joint_pos = COCOVelocityActionsCfg.joint_pos
...
class RewardsCfg:
    ...
    target_vel_rew = RewTerm(
        func=nav_mdp.target_vel_reward, weight=10.0, params={"command_name": "pose_command"}
    )
...
@configclass
class CostnavIsaaclabV2EnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for COCO robot navigation environment with custom map."""

    # Scene settings - reduced num_envs for complex map
    scene: CostnavIsaaclabV2SceneCfg = CostnavIsaaclabV2SceneCfg(num_envs=64, env_spacing=4.0)
    # Basic settings
