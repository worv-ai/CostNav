...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            echo "Usage: $0 --input <frames_dir> [--output <out_dir>] [--fps 24] [--timestep 250]"
            exit 0;;
...
if [ -s after_list.txt ]; then
    ffmpeg -y -f concat -safe 0 -i after_list.txt \
        ...
        -r "${FIXER_FPS}" -c:v libx264 -pix_fmt yuv420p -movflags +faststart "${OUTPUT_DIR}/after.mp4"

    ffmpeg -y -f concat -safe 0 -i before_list.txt \
        -vf "settb=AVTB,setpts=N/${FIXER_FPS}/TB" \
        -r "${FIXER_FPS}" -c:v libx264 -pix_fmt yuv420p -movflags +faststart "${OUTPUT_DIR}/before.mp4"

    ffmpeg -y -f concat -safe 0 -i before_list.txt \
        -f concat -safe 0 -i after_list.txt \
        -filter_complex "[0:v][1:v]hstack,settb=AVTB,setpts=N/${FIXER_FPS}/TB" \
        -r "${FIXER_FPS}" -c:v libx264 -pix_fmt yuv420p -movflags +faststart "${OUTPUT_DIR}/comparison.mp4"
fi
