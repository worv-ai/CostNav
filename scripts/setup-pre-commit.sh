...
# Update the pre-push hook to support both environments
PUSH_HOOK_FILE=".git/hooks/pre-push"

if [ -f "$PUSH_HOOK_FILE" ]; then
    echo "Updating pre-push hook to support both devcontainer and local environments..."

    cat > "$PUSH_HOOK_FILE.tmp" << 'EOF'
#!/usr/bin/env bash
# File generated by pre-commit: https://pre-commit.com
# ID: 138fd403232d2ddd5efb44317e38bf03
...
EOF
    ...
    mv "$PUSH_HOOK_FILE.tmp" "$PUSH_HOOK_FILE"
    chmod +x "$PUSH_HOOK_FILE"

    echo "✓ Pre-push hook updated successfully"
else
    echo "Warning: Pre-push hook file not found at $PUSH_HOOK_FILE"
    exit 1
fi

echo ""
echo "✓ Pre-commit and pre-push hooks setup complete!"
echo ""
echo "The hooks will now run automatically on git commit and git push."
echo "To run hooks manually on all files: $PYTHON_CMD -m pre_commit run --all-files"
echo "To run pre-push checks manually: $PYTHON_CMD -m pre_commit run --hook-stage pre-push --all-files"
