...
| ------------------------------- | --------------------------- | ------ | ----------------------------- |
| `data_processing/`, `training/` | SLURM cluster or bare-metal | ≥3.11  | `uv sync` from `il_training/` |

**Development & Training** uses `uv` directly on the host (bare-metal or SLURM cluster).
This is CPU-friendly for data processing and leverages GPU for model training without
Docker overhead.

**Application (Deployment)** of trained models runs inside Docker containers.
IL evaluation (inference + ROS2 nodes) is a separate package at
[`../il_evaluation/`](../il_evaluation/README.md), built with `Dockerfile.ros_torch`.
...
This installs `il_training` as an editable package so all imports
(`from il_training.data_processing.converters...`, `from il_training.training...`) work.
...
| Concern    | il_training               | il_evaluation                       |
| ---------- | ------------------------- | ----------------------------------- |
| Runtime    | Slurm / bare-metal (`uv`) | Docker (`Dockerfile.ros_torch`)     |
| Python     | ≥3.11                     | 3.10 (ROS2 Jazzy constraint)        |
| numpy      | ≥2.0 OK                   | <2 (cv_bridge)                      |
| Extra deps | ray, wandb, hydra         | rclpy, cv_bridge, casadi            |
| Install    | `uv sync`                 | `uv pip install -e .` inside Docker |

Evaluation has its own `pyproject.toml` and lives in a separate folder
([`costnav_isaacsim/il_evaluation/`](../il_evaluation/README.md)) to decouple the Docker/ROS2 lifecycle.
...
```bash
uv run python -m il_training.data_processing.converters.rosbag_to_mediaref \
...
See [`../il_evaluation/README.md`](../il_evaluation/README.md) for documentation on running trained IL policies in Isaac Sim.
