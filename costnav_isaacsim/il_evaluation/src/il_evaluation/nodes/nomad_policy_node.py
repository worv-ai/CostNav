...
class NoMaDPolicyNode(Node):
    ...
    * **topomap** — follow a topological map (directory of numbered PNGs).
    ...
    From model_config (nomad_eval.yaml):
    ...
    def __init__(
        ...
        super().__init__("nomad_policy_node")
        ...
        # Initialize NoMaD agent
        self.get_logger().info(f"Loading NoMaD model from {checkpoint}")
        self.agent = NoMaDAgent(
            model_path=checkpoint,
            model_config_path=model_config,
            device=device,
        )
        self.agent.reset(batch_size=1)
        self.get_logger().info("NoMaD agent initialized successfully")
        ...
        self.get_logger().info(
            f"NoMaD policy node started (goal_type={self.goal_type}). Inference rate: {self.inference_rate} Hz"
        )
    ...
    def _load_topomap(self, topomap_dir: str, goal_node: int) -> None:
        ...
        ``TopomapGenerator`` and expected by NoMaD's ``navigate.py``.
    ...
    def _handle_reset_agent(self, _request, response):
        ...
        try:
            ...
            response.message = "NoMaD agent memory queue reset successfully"
    ...
    def enable_callback(self, msg: Bool):
        ...
        self.get_logger().info(f"NoMaD policy {status}")
    ...
    def _inference_topomap(self) -> None:
        ...
        Implements the navigation loop from NoMaD's ``navigate.py``:
    ...
    def _publish_trajectory(
        ...
        if self._trajectory_publish_count % self._log_interval == 0:
            ...
            self.get_logger().info(
                f"[NoMaD] #{self._trajectory_publish_count}: traj_last=({last_wp[0]:.2f}, {last_wp[1]:.2f}){extra}"
            )

    def trajectory_to_path(self, trajectory: np.ndarray) -> Path:
        """Convert NoMaD trajectory output to nav_msgs/Path message.
...
def parse_args():
    ...
    Most parameters are read from model_config (nomad_eval.yaml) and
    ...
    """
    parser = argparse.ArgumentParser(description="NoMaD ROS2 Policy Node for CostNav")
    parser.add_argument(
        "--checkpoint",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=NoMaDPolicyNode.VALID_GOAL_TYPES,
        help="Override navigation mode from nomad_eval.yaml (no_goal | image_goal | topomap)",
    )
    parser.add_argument(
        "--topomap_dir",
        type=str,
...
def main():
    ...
    try:
        ...
        print(f"Error starting NoMaD policy node: {e}")
        raise
    finally:
        rclpy.shutdown()
