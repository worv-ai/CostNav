...
"""Shared ROS2 policy node for CostNav IL baselines.

This module contains the shared inference, topomap navigation, and ROS
plumbing used by ViNT, GNM, and NoMaD policy nodes. Model-specific
wrappers should supply the agent class and model name.
"""
...
class BasePolicyNode(Node):
    """ROS2 Node for IL policy inference (shared across models)."""

    #: Valid navigation mode values for ``goal_type``.
    VALID_GOAL_TYPES = ("no_goal", "image_goal", "topomap")

    #: Model-agnostic ROS topics/services.
    MODEL_ENABLE_TOPIC = "/model_enable"
    MODEL_RESET_SERVICE = "/model_reset_agent"
    MODEL_LOAD_TOPOMAP_SERVICE = "/model_load_topomap"
    MODEL_TRAJECTORY_TOPIC = "/model_trajectory"
    MODEL_TRAJECTORY_MAP_TOPIC = "/model_trajectory_map"
    MODEL_REACHED_GOAL_TOPIC = "/model_reached_goal"
    MODEL_GOAL_IMAGE_TOPIC = "/model_goal_image"
    MODEL_LOCALIZATION_IMAGE_TOPIC = "/model_localization_image"
    ...
    def __init__(
        ...
        *,
        ...
        self.get_logger().info(f"{self.model_name} agent initialized successfully")
        ...
        # triggered later via the /model_load_topomap service.
        ...
        if self.goal_type == "topomap":
            self._load_topomap_service = self.create_service(
                Trigger, self.MODEL_LOAD_TOPOMAP_SERVICE, self._handle_load_topomap
            )
        ...
        # Publishers
        self.trajectory_pub = self.create_publisher(Path, self.MODEL_TRAJECTORY_TOPIC, 10)
        self.trajectory_map_pub = self.create_publisher(Path, self.MODEL_TRAJECTORY_MAP_TOPIC, 10)
        self.reached_goal_pub = self.create_publisher(Bool, self.MODEL_REACHED_GOAL_TOPIC, 10)
        ...
        if self.visualize_debug_images:
            self._goal_image_pub = self.create_publisher(Image, self.MODEL_GOAL_IMAGE_TOPIC, 10)
            self._localization_image_pub = self.create_publisher(Image, self.MODEL_LOCALIZATION_IMAGE_TOPIC, 10)
            self.get_logger().info(
                "Debug image visualization enabled. Publishing to: "
                f"{self.MODEL_GOAL_IMAGE_TOPIC}, {self.MODEL_LOCALIZATION_IMAGE_TOPIC}"
            )
        ...
        self.get_logger().info(
            f"{self.model_name} policy node started (goal_type={self.goal_type}). "
            f"Inference rate: {self.inference_rate} Hz"
        )
        self.get_logger().info(f"Subscribing to: {image_topic}")
        self.get_logger().info(f"Publishing trajectory to: {self.MODEL_TRAJECTORY_TOPIC}")
        if self.goal_type == "topomap":
            self.get_logger().info(
                "Topomap mode enabled. Waiting for "
                f"{self.MODEL_LOAD_TOPOMAP_SERVICE} service call (topomap_dir={self._topomap_dir})"
            )
    ...
    def _load_topomap(self, topomap_dir: str, goal_node: int) -> None:
        ...
        ``TopomapGenerator`` and expected by the model's ``navigate.py``.
        ...
        try:
            ...
            if self._goal_image_pub is not None:
                try:
                    ...
                    self.get_logger().info(f"Published received goal image to {self.MODEL_GOAL_IMAGE_TOPIC}")
    ...
    def enable_callback(self, msg: Bool):
        ...
        ``/model_enable`` at the start of every mission (after generating a
        fresh topomap), so this is the reliable per-mission reload hook.
        """
        was_enabled = self.enabled
        self.enabled = msg.data
        status = "enabled" if self.enabled else "disabled"
        self.get_logger().info(f"{self.model_name} policy {status}")
    ...
    def _inference_topomap(self) -> None:
        ...
        Implements the navigation loop from the baseline ``navigate.py``:
    ...
    def _publish_trajectory(
        ...
        if self._trajectory_publish_count % self._log_interval == 0:
            ...
            self.get_logger().info(
                f"[{self.model_name}] #{self._trajectory_publish_count}: "
                f"traj_last=({last_wp[0]:.2f}, {last_wp[1]:.2f}){extra}"
            )

    def trajectory_to_path(self, trajectory: np.ndarray) -> Path:
        """Convert trajectory output to nav_msgs/Path message.
    ...
    def _publish_trajectory_map(self, base_link_path: Path) -> None:
        ...
        to every pose in *base_link_path*.  Published on
        ``/model_trajectory_map``.
        """
        try:
...
def build_arg_parser(model_name: str, model_config_name: str) -> argparse.ArgumentParser:
    """Build command line argument parser for a model-specific policy node."""
    parser = argparse.ArgumentParser(description=f"{model_name} ROS2 Policy Node for CostNav")
    parser.add_argument(
        "--checkpoint",
        type=str,
        required=True,
...
def run_policy_node(node_cls, args, node_label: str) -> None:
    """Run a policy node with the shared log-level setup and shutdown handling."""
    rclpy.init()
    ...
    try:
        ...
        print(f"Error starting {node_label} policy node: {e}")
        raise
    finally:
        rclpy.shutdown()
