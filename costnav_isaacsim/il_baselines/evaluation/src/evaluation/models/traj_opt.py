# Copyright (c) 2023 Fan Yang, ETH Zurich
# Copyright (c) 2026 CostNav Authors
# Licensed under the MIT License

"""Trajectory optimization utilities using cubic spline interpolation."""

import torch

torch.set_default_dtype(torch.float32)


class CubicSplineTorch:
    """Cubic spline interpolation in PyTorch.

    Reference: https://stackoverflow.com/questions/61616810/
    how-to-do-cubic-spline-interpolation-and-integration-in-pytorch
    """

    def __init__(self):
        pass

    def h_poly(self, t: torch.Tensor) -> torch.Tensor:
        """Compute Hermite polynomial basis functions."""
        alpha = torch.arange(4, device=t.device, dtype=t.dtype)
        tt = t[:, None, :] ** alpha[None, :, None]
        A = torch.tensor(
            [[1, 0, -3, 2], [0, 1, -2, 1], [0, 0, 3, -2], [0, 0, -1, 1]],
            dtype=t.dtype,
            device=t.device,
        )
        return A @ tt

    def interp(self, x: torch.Tensor, y: torch.Tensor, xs: torch.Tensor) -> torch.Tensor:
        """Interpolate values at query points xs.

        Args:
            x: Control point x-coordinates [B, N].
            y: Control point values [B, N, D].
            xs: Query x-coordinates [B, M].

        Returns:
            Interpolated values at xs of shape [B, M, D].
        """
        m = (y[:, 1:, :] - y[:, :-1, :]) / torch.unsqueeze(x[:, 1:] - x[:, :-1], 2)
        m = torch.cat([m[:, None, 0], (m[:, 1:] + m[:, :-1]) / 2, m[:, None, -1]], 1)
        idxs = torch.searchsorted(x[0, 1:], xs[0, :])
        dx = x[:, idxs + 1] - x[:, idxs]
        hh = self.h_poly((xs - x[:, idxs]) / dx)
        hh = torch.transpose(hh, 1, 2)
        out = hh[:, :, 0:1] * y[:, idxs, :]
        out = out + hh[:, :, 1:2] * m[:, idxs] * dx[:, :, None]
        out = out + hh[:, :, 2:3] * y[:, idxs + 1, :]
        out = out + hh[:, :, 3:4] * m[:, idxs + 1] * dx[:, :, None]
        return out


class TrajOpt:
    """Trajectory optimization using cubic spline interpolation."""

    def __init__(self):
        self.cs_interp = CubicSplineTorch()

    def TrajGeneratorFromPFreeRot(self, preds: torch.Tensor, step: float = 0.1) -> torch.Tensor:
        """Generate smooth trajectory from predicted waypoints.

        Args:
            preds: Predicted waypoints of shape [B, N, D] where D includes (x, y, ...).
            step: Interpolation step size (default: 0.1).

        Returns:
            Interpolated trajectory of shape [B, M, D] where M is determined by step.
        """
        batch_size, num_p, dims = preds.shape

        # Prepend origin point (0, 0, ...) to predictions
        points_preds = torch.cat(
            (
                torch.zeros(batch_size, 1, dims, device=preds.device, requires_grad=preds.requires_grad),
                preds,
            ),
            axis=1,
        )
        num_p = num_p + 1

        # Generate query points
        xs = torch.arange(0, num_p - 1 + step, step, device=preds.device)
        xs = xs.repeat(batch_size, 1)

        # Generate control points
        x = torch.arange(num_p, device=preds.device, dtype=preds.dtype)
        x = x.repeat(batch_size, 1)

        # Interpolate
        waypoints = self.cs_interp.interp(x, points_preds, xs)
        return waypoints  # [B, M, D] in R^3 (x, y, theta)
