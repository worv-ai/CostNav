...
def generate_launch_description():
    ...
    joy_node_name_arg = DeclareLaunchArgument(
        ...
        description='Name of the joy node'
    )
    
    joy_deadzone_arg = DeclareLaunchArgument(
        'joy_deadzone',
        default_value='0.12',
        description='Deadzone for joystick'
    )
    
    use_teleport_arg = DeclareLaunchArgument(
        'use_teleport',
        default_value='true',
        description='Enable teleport functionality'
    )
    
    use_clock_arg = DeclareLaunchArgument(
        'use_clock',
        default_value='false',
        description='Use clock topic'
    )
    
    use_people_pose_arg = DeclareLaunchArgument(
        'use_people_pose',
        default_value='true',
        description='Use people pose topic'
    )
    
    use_wheel_odom_arg = DeclareLaunchArgument(
        'use_wheel_odom',
        default_value='false',
        description='Use wheel odometry'
    )
    
    auto_restart_on_collision_arg = DeclareLaunchArgument(
        'auto_restart_on_collision',
        default_value='true',
        description='Auto restart on collision'
    )
    
    use_control_topic_arg = DeclareLaunchArgument(
        'use_control_topic',
        default_value='false',
        description='Use control topic mode'
    )
    
    frame_id_arg = DeclareLaunchArgument(
        'frame_id',
        default_value='teleop',
        description='Frame ID for teleop'
    )
